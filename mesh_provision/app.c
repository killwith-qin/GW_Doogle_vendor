...
void User_ADV_RECEIVE_Process(event_adv_report_t *pa)
{
    
        memcpy((u8 *)&Unfilter_ADV_Message,pa->data,11);
        if( (Unfilter_ADV_Message.cmd == USER_SEND_COMMAND_TEST) && ( GW_Role == GW_PASSIVE ) )
        {
            memcpy((u8 *)&Get_ADV_Message,pa->data,11);
            LOG_USER_MSG_INFO((u8 *)&Get_ADV_Message, 11,"ADV mesaage: ",0);
        }
        if(Unfilter_ADV_Message.cmd == REQUEST_AS_PASSIVE_CMD)
		{
            memcpy((u8 *)&Get_ADV_Message,pa->data,11);
			LOG_USER_MSG_INFO(&pa->len, 1,"ADV length: ",0);
			LOG_USER_MSG_INFO((u8 *)&Get_ADV_Message, 11,"ADV mesaage: ",0);
		}
		memcpy((u8 *)&Unfilter_ADV_Message,&Init_ADV_Message,11);
