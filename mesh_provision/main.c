...
_attribute_ram_code_ int main (void)    //must run in ramcode
{
	...
	//Qin Wei
	//gpio_set_output_en(GPIO_PA4, 1); //configuration has been changed, OEN is ENABLE in here again.
...
/*Changed by QW :FOR Mesh_GW_Doogle need write MID to this address*/
    flash_write_page(CFG_ADR_MAC_512K_FLASH,6,Mesh_GW_MacID);

    irq_enable();
    LOG_USER_MSG_INFO(Mesh_GW_MacID, 6,"Mesh_GW_MacID:",0);
	while (1) {
#if (MODULE_WATCHDOG_ENABLE)
		wd_clear(); //clear watch dog
#endif
